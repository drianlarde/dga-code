...
def main():
    ...
    if st.button("Run Distributed Genetic Algorithm", key="run_dga_button"):
        ...
        courses = list(st.session_state['courses_units'].keys())
        ...
        courses_units_adjusted = {course: min(units, 3) for course, units in st.session_state['courses_units'].items()}
        ...
        for island_name in islands.keys():
            islands[island_name] = initialize_population(CHROMOSOMES_PER_ISLAND, st.session_state.faculty_list, courses_units_adjusted, courses, rooms, days, time_slots)
