...
# List of classrooms
classrooms = ["Com Lab 1", "Com Lab 2", "Com Lab 3", "Com Lab 4"] + \
             [f"GV 30{i}" for i in range(1, 8)] + \
             [f"GCA 30{i}" for i in range(1, 6)]

# Load the data
df = pd.read_csv(r"C:\Users\Admin\Downloads\combinations.csv")

# Assigning a random classroom to each record
df["classrooms"] = [random.choice(classrooms) for _ in range(len(df))]

# Assigning faculty as either full-time or part-time
df["faculty"] = np.where(df.index % 2, "Full Time", "Part Time")

df['timeslot'] = df['start_time'] + ' - ' + df['end_time']

# Sample the DataFrame to create df_sampled
try:
...
class Schedule:
    ...
    def __str__(self):
        ...
        for assignment in self.assignments:
            schedule_details += f"Course: {assignment['course']}, Faculty: {assignment['faculty']}, Timeslot: {assignment['timeslot']}, Classrooms: {assignment['classrooms']}\n"
    ...
    def fitness(self):
        ...
        classroom_usage = {}
        for a in self.assignments:
            classroom_usage[a['classrooms']] = classroom_usage.get(a['classrooms'], 0) + self.calculate_duration(a['start_time'], a['end_time'])
        ...
        for i in range(len(self.assignments)):
            for j in range(i + 1, len(self.assignments)):
                if self.assignments[i]['timeslot'] == self.assignments[j]['timeslot'] and \
                        self.assignments[i]['classrooms'] == self.assignments[j]['classrooms']:
                    fitness_score -= 200
                    break
...
def generic_ga(population_size, max_generations, df, teacher_type, max_hours=None, unavailable_days=None):
    ...
    location_to_index_map = {classroom: index for index, classroom in enumerate(df['classrooms'].unique(), start=1)}
